...
export class ConvertArraysToJson20250926000001 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`experience\` MODIFY \`technologies\` JSON NULL`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE \`project\` ADD \`category\` VARCHAR(50) NULL DEFAULT 'Personal'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`project\` ADD \`order\` INT NULL DEFAULT 0`,
    );

    // Add new Experience fields for education
    await queryRunner.query(
      `ALTER TABLE \`experience\` ADD \`institution\` VARCHAR(255) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`experience\` ADD \`degree\` VARCHAR(255) NULL`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`project\` MODIFY \`techStack\` TEXT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`experience\` MODIFY \`technologies\` TEXT NULL`,
    );
  }
}
